 ...
 * a specified region, upon the client's request.
 */
export const clusterRequestHandlerOptimized = functions.https.onRequest(
  async (request, response) => {
    ...
    try {
      const clusters = await DB.computeClustersInRegionOptimized(regionObj);
      response.status(200).send(clusters);
      return;
    } catch (err) {
      response.status(400).send(err.message || 'Could not get clusters due to an unknown reason.');
    }
  }
);
